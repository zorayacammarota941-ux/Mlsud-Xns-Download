...
pub(super) async fn start_with_existing_service(config_file: &PathBuf) -> Result<()> {
    ...
    let response = clash_verge_service_ipc::start_clash(&payload)
        .await
        .context("无法连接到 Mlsud Xns Service")?;
...
mod tests {
    ...
    fn detects_app_translocation_paths() {
        let path = Path::new(
            "/private/var/folders/example/T/AppTranslocation/123/d/Mlsud Xns.app/Contents/MacOS/Mlsud Xns",
        );
    ...
    fn extracts_app_bundle_name_from_executable_path() {
        let path = Path::new("/Applications/Mlsud Xns.app/Contents/MacOS/Mlsud Xns");

        assert_eq!(
            macos_app_bundle_name(path).as_deref(),
            Some(std::ffi::OsStr::new("Mlsud Xns.app"))
        );
    }
    ...
    fn resolves_existing_core_path_from_install_roots() -> std::io::Result<()> {
        ...
        let core_dir = root.join("Mlsud Xns.app").join("Contents").join("MacOS");
        ...
        let resolved = macos_core_path_in_install_roots(
            std::ffi::OsStr::new("Mlsud Xns.app"),
            "verge-mihomo",
            [root.as_path()],
        );
